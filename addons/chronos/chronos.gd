...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("[Chronos] Session start recorded: day %d, time %.2f" % [_session_start_day, _session_start_time])
...
## Returns true if session start time has been initialized.
func is_session_initialized() -> bool:
	return _session_initialized


## Returns true if time cannot rewind further (at or before session start).
## Use this to detect when rewinding should stop.
func is_at_session_start() -> bool:
	if not _session_initialized:
		return false
	var same_day: bool = day_count == _session_start_day
	var at_start_time: bool = time <= _session_start_time + 0.01
	return same_day and at_start_time
